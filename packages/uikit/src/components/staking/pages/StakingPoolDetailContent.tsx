...
interface StakingPoolDetailContentProps {
    poolAddress?: string;
    renderHeader: (props: {
        title: string;
        hasMultipleStakingPositions: boolean;
        isError: boolean;
    }) => ReactNode;
}
...
export const StakingPoolDetailContent = ({
    ...
    const stakingPositionsCount = useMemo(() => {
        ...
        const positionsCount = portfolio.stakingPositions.length;
        const stakingTokenBalancesCount = portfolio.tokenBalances.filter(
            token => token.stakingPool
        ).length;
        return positionsCount + stakingTokenBalancesCount;
    }, [portfolio]);
    ...
    const displayAmount = stakedAmount
        ? formatTokenDisplay(stakedAmount, BRAND_CONFIG.coinSymbolWithEx)
    ...
    const formattedCycleDate = useDateFormat(
        cycleEndDate,
        useMemo<Intl.DateTimeFormatOptions>(
            () => ({
                month: 'long',
                day: 'numeric',
                year: 'numeric',
    ...
    const hasMultipleStakingPositions = stakingPositionsCount > 1;

    if (isPoolError) {
        return (
            <>
                {renderHeader({ title: '', hasMultipleStakingPositions, isError: true })}
                <ContentArea>
                    <InfoRow>
